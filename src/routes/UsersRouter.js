...
router.get('/users/:id/bac', getUserBAC);
...
async function getUserByUsername(request, response, next) {
    try {
        let user = await UserModel.find({username: request.params.username}).exec();
        if (user.length >= 1) {
            response.statusCode = statusOK;
            response.send(user[0]);
        } else {
            response.statusCode = statusError;
            next("No user found for get user by username");
        }
    } catch (e) {
        next(e);
    }
...
async function getUserBAC(request, response, next) {
    try {
        let user = await UserModel.find({_id: request.params.id}).exec();
        if (user.length >= 1) {
            response.statusCode = statusOK;
            response.send({bac: user[0].bac});
        } else {
            response.statusCode = statusError;
            next("No user found for get user bac");
        }
    } catch (e) {
        next(e);
    }
